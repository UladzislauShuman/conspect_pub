...
- Забытое
    - зачем Spring?
    - как у него дела? какие новые фичи добавили
    - минусы Spring
    - Spring Boot (Конвенция, стартеры, conditional beans)
    - депрейтнули Spring Factories
    - Как инджектить Prototype в Singltone через @Lookup
    - скоупы. особенности
    - как работает autowired?
    - что можно использовать для работы с БД, Кафка, Рест, Сап(?)
    - RestTemplete
    - зачем автоконфигурация
    - как Spring ищет классы
    - Аннотации Qualifier, Primary, ComponentScan, RestController, Controller, GetMapping, PostMapping, PathVariable, RequestParam, RequestBody
    - Best Practice
