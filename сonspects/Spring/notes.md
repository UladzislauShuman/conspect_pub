...
так же я мог некоторые темы раскрыть лучше засчет лекций с Т-Академии. второй семестр начался с семенара по [Spring](./also/1С.%20Spring%20Core:%20IoC%20и%20конфигурация/notes.md). Поэтому -- прочитайте этот файл И возможно следующие лекции тоже про Spring
...
## Содержание
- в [этом](./also/1С.%20Spring%20Core:%20IoC%20и%20конфигурация/notes.md) файле разобраны
    - IoC
...
- Что такое Spring
    - [минусы Srping](#минусы-spring)
    - [Spring Boot. Стартеры](#spring-boot-конвенция-стартеры-conditional-beans) 
- Основа
    ...
    - [Как создается Proxy](#как-создается-proxy)
...
- Аннотации
    ...
    - [Как заинджектить Prototype в Singletone. @Lookup](#как-заинджектить-prototype-в-singletone-lookup)
    ...
    - [GetMapping, PostMapping и другие](#getmapping-postmapping-и-другие)
    - [Аннотации PathVariable, RequestParam, RequestBody](#аннотации-pathvariable-requestparam-requestbody)
...
- Без категории
    - [Как можно исполнить код на старте приложения](#как-можно-исполнить-код-на-старте-приложения)
- Надо дописать
    - депрейтнули Spring Factories
    - что можно использовать для работы с БД, Кафка, Рест, Сап(?)
    - RestTemplete
    - Написал классик open, вижу, что транзакция не создается. Что может быть не так? Дернули не тот экземпляр, без обертки.
    - другие способы реализовать DI

## Вопросы

### минусы Spring
- Медленный старт: при запуске Spring сканирует весь проект, ищет аннотации, создает объекты (бины) и связывает их. Крупные приложения могут запускаться по несколько минут.

- Высокое потребление памяти: из-за активного использования рефлексии (Reflection API) и хранения метаданных в оперативной памяти, пустое приложение на Spring Boot требует гораздо больше ОЗУ, чем аналоги на Go, Node.js или легковесных Java-фреймворках (Ktor, Quarkus).

- Сложная отладка: из-за магии DI, Stack Trace при ошибке может состоять из сотен строк системного кода Spring.

### Spring Boot (Конвенция, стартеры, conditional beans)

я предлагаю -- вот это [видео](https://www.youtube.com/watch?v=yy43NOreJG4) и также [как сделать стартер](https://struchkov.dev/blog/ru/create-spring-boot-starter/)

### Жизненный цикл Spring Bean

Лучше посмотреть Spring потрошителя, честно. Детальнее и Понятнее будет
...
указывает фреймворку, в каких пакетах нужно искать классы, помеченные аннотациями @Component, @Service, @Repository или @Controller, чтобы автоматически создать из них бины и добавить в контекст приложения.

Без нее механизм @Autowired работать не будет, так как Spring просто не увидит классы.

- Как она работает
    - При старте приложения Spring сканирует указанные пакеты.
    - Он читает байт-код каждого класса и ищет стереотипные аннотации (наследники @Component).
    - Найденные классы регистрируются в ApplicationContext как управляемые Spring бины.

В Spring Boot ее редко пишут вручную. Она уже встроена внутрь главной аннотации @SpringBootApplication. По умолчанию Spring Boot начинает сканирование с того пакета, где находится запускающий класс с main(), и идет по всем его подпакетам.

парамЕтры
- basePackages (или value): Указывает имена пакетов для сканирования строками.
- basePackageClasses: Более безопасный (type-safe) способ. Указывает классы, из пакетов которых нужно начать сканирование. Защищает от опечаток при рефакторинге.
- Фильтры (includeFilters / excludeFilters): Позволяют гибко включить или исключить определенные классы из сканирования (например, по регулярному выражению, типу или кастомной аннотации).
...
прокси-объекты создаются автоматически на этапе BeanPostProcessor, то есть после инициализации бина, если класс использует механизмы AOP: транзакции (@Transactional), безопасность (@PreAuthorize), кэширование (@Cacheable) или асинхронность (@Async).

использует две разные технологии: JDK Dynamic Proxy и CGLIB Proxy.

#### JDK Dynamic Proxy (На основе интерфейсов)
Использует механизмы Java Reflection API.

* Условие создания: Класс целевого бина обязательно должен реализовывать хотя бы один интерфейс.
* Как работает: Spring создает в памяти новый класс-суррогат, который реализует тот же интерфейс, что и ваш бин. Он перехватывает вызовы методов через обработчик InvocationHandler.
* Ограничение: Невозможно проксировать методы, которых нет в интерфейсе.

#### CGLIB Proxy (На основе наследования)
Использует генерацию байт-кода на лету с помощью библиотеки ByteBuddy

* Условие создания: Класс не реализует интерфейсы, либо принудительно включен режим proxyTargetClass = true.
* Как работает: Spring создает в памяти новый класс, который является прямым наследником (подклассом) вашего класса. Перехват вызовов идет через переопределение методов.
* Ограничение: Класс и его методы не должны быть final (иначе от них нельзя наследоваться).

CGLIB используется по умолчанию для всех бинов, даже если у них есть интерфейсы. Это сделано для стабильности, чтобы избежать ошибок приведения типов (ClassCastException).

------------------------------
## 🔄 Жизненный цикл: Как именно создается прокси
Процесс создания встроен в стандартный жизненный цикл бина Spring:

   1. Сканирование: Сборщик контекста находит класс (например, @Service).
   2. Инстанцирование: Spring создает обычный Java-экземпляр вашего класса (Target Object).
   3. Настройка: Внедряются зависимости (@Autowired).
   4. Инициализация: Отрабатывает @PostConstruct.
   5. Этап BPP (BeanPostProcessor): Вступает в дело AbstractAutoProxyCreator. Он сканирует методы бина на наличие @Transactional, @Async и т.д.
   6. Генерация прокси: Если маркеры найдены, Spring создает обертку (Proxy) с помощью CGLIB или JDK Dynamic Proxy, помещает внутрь нее ваш оригинальный объект и возвращает в контекст именно прокси.

Когда другой бин вызывает метод вашего сервиса, он вызывает его у прокси-объекта. Прокси выполняет сквозную логику (открывает транзакцию), затем вызывает метод реального объекта, а после — закрывает транзакцию.

#### Самовызов

магия работает только тогда, когда метод вызывают снаружи через объект-прокси, вызов метода внутри того же класса ломает логику.

для этого можно , например, лениво инджектить самого себя
...
это аннотация в Spring, которая отвечает за автоматическое Dependency Injection бина в компонент приложения.

#### Как работает 
За обработку аннотации отвечает AutowiredAnnotationBeanPostProcessor. Процесс поиска и внедрения зависимости проходит по следующим шагам:

   1. Определение типа: Контейнер определяет тип объекта, который необходимо внедрить.
   2. Поиск в контексте: Контейнер ищет соответствующий тип бина в контексте (он же контейнер).
   3. Разрешение неоднозначности через @Primary: Если найдено несколько кандидатов, и один из них помечен аннотацией @Primary, то внедряется именно он.
   4. Разрешение неоднозначности через @Qualifier: Если используется аннотация @Qualifier, контейнер берет информацию из нее, чтобы точно понять, какой компонент внедрять.
   5. Поиск по имени: В противном случае (если нет @Primary и @Qualifier), контейнер пытается внедрить бин, основываясь на его имени или ID.
   6. Выброс исключения: Если ни один из способов не сработал и подходящий бин не найден, выбрасывается исключение.

#### Основные параметры и ограничения

- В аннотации есть один параметр required = true/false, указывающий, обязательно ли делать DI. По умолчанию он равен true. Если выставить false, то в случае отсутствия нужного бина контейнер не выбросит исключение, а просто оставит поле со значением null.
- Аннотации @Autowired, @Inject, @Value и @Resource обрабатываются реализациями BeanPostProcessor в Spring. Это означает, что невозможно применять эти аннотации внутри ваших собственных типов BeanPostProcessor или BeanFactoryPostProcessor. На этом этапе процессоры аннотаций еще не активны. Такие типы должны быть "связаны" явным образом с помощью XML или метода @Bean.
* Если объекты циклически ссылаются друг на друга, аннотацию @Autowired нельзя ставить над конструктором, иначе приложение упадет при запуске.

#### 3 варианта использования @Autowired
...
- Constructor
    ...
    - минусы
        - циклические зависимости сложнее обработать
- Setter
    - Преимущества
...
- Field
    - Преимущества
        - Простота
    - ➖ Минусы:
        - Существует способ (путем вызова конструктора по умолчанию) создать объект с использованием new в состоянии, когда ему не хватает обязательных зависимостей. Его дальнейшее использование приведет к NullPointerException.
        - Такой класс не может быть использован вне DI-контейнеров (например, в модульных тестах), и нет способа, кроме рефлексии, предоставить ему зависимости.
        - В отличие от конструктора, этот способ нельзя использовать для присвоения зависимостей final-полям. Это приводит к тому, что ваши объекты становятся изменяемыми, теряя неизменность.

### Как заинджектить Prototype в Singletone. @Lookup

это аннотация в Spring, которая реализует паттерн Method Injection

это способ попросить Spring переопределить метод в вашем классе nfr, чтобы при каждом вызове этого метода Spring лез в свой контекст и доставал оттуда актуальный бин.
...
public class MySingleton {
    ...
    public void doSomething() {
        // каждый вызов метода getPrototype() вернет НОВЫЙ экземпляр!
        MyPrototype prototypeInstance = getPrototype();
        System.out.println("Prototype ID: " + prototypeInstance.getId());
    }

    @Lookup
    public MyPrototype getPrototype() {
        // Тело метода оставляем пустым или возвращаем null.
        // Spring CGLIB переопределит этот метод на лету и подставит логику обращения к контексту.
        return null; 
    }
}
...
public class MySingletonWithFactory {
    ...
    public void doSomething() {
        MyPrototype prototypeInstance = prototypeFactory.getObject();
        System.out.println("Prototype ID: " + prototypeInstance.getId());
    }
}
```
...
// тоже лучше переписать или пересмотреть что-то другое
[пример](https://habr.com/ru/articles/682362/)
...
### GetMapping, PostMapping и другие

специализированные сокращения над @RequestMapping

связывают конкретный HTTP-метод и URL-адрес с Java-методом в контроллере.

работают только внутри классов, помеченных как @Controller или @RestController.

| Аннотация | HTTP Метод | Назначение (в REST) | Идемпотентность |
|---|---|---|---|
| @GetMapping | GET | Получение данных или ресурса | Да (Безопасный) |
| @PostMapping | POST | Создание нового ресурса | Нет |
| @PutMapping | PUT | Полное обновление (замена) ресурса | Да |
| @DeleteMapping | DELETE | Удаление ресурса | Да |
| @PatchMapping | PATCH | Частичное обновление ресурса | Нет |

### Аннотации PathVariable, RequestParam, RequestBody

используются в Spring MVC контроллерах для извлечения данных из HTTP-запроса

- @PathVariable
    - Откуда берет данные: Прямо из URL-адреса (пути) запроса.
    - Для чего нужна: Для извлечения идентификаторов ресурсов в REST-стиле.
    - Пример URL: /users/42
    - пример
    ```java
...
- @RequestParam
    - Откуда берет данные: Из строки параметров URL (после знака ?) или из данных отправленной HTML-формы.
    - Для чего нужна: Для фильтрации, пагинации, сортировки или передачи необязательных параметров.
    - Пример URL: /users?page=2&status=active
    - пример
    ```java
    @GetMapping("/users")public List<User> getUsers(@RequestParam int page, @RequestParam String status) { ... }
    ```

- @RequestBody
    - Откуда берет данные: Из тела (body) HTTP-запроса (обычно в формате JSON или XML).
    - Для чего нужна: Для передачи сложных объектов и сущностей при создании или обновлении данных (методы POST, PUT, PATCH). Spring автоматически десериализует JSON в Java-объект с помощью Jackson.
    - пример
    ```java
    @PostMapping("/users")public User createUser(@RequestBody User newUser) { ... }
    ```
...
### Как можно исполнить код на старте приложения


#### Интерфейс CommandLineRunner

специально для выполнения кода при старте. есть доступ к сырым аргументам командной строки, с которыми было запущено приложение.

- Как работает: Spring находит все бины, реализующие этот интерфейс, и запускает их метод run() после того, как контекст полностью поднят.
- Пример:
```java
@Componentpublic class MyCommandLineRunner implements CommandLineRunner {
    @Override
    public void run(String... args) throws Exception {
        System.out.println("Приложение успешно запущено! Переданные аргументы: " + Arrays.toString(args));
        // Ваша логика (например, миграция данных или проверка папок)
    }
}
```
...
идентичен CommandLineRunner, но предоставляет аргументы командной строки в виде удобного объекта ApplicationArguments, разбивая их на ключи и значения (например, --port=8080).

```java
@Componentpublic class MyApplicationRunner implements ApplicationRunner {
    @Override
    public void run(ApplicationArguments args) throws Exception {
        if (args.containsOption("my-custom-flag")) {
            ...
        }
    }
}
...
@Component
public class MyReadyEventListener {
    @EventListener(ApplicationReadyEvent.class)
